  ...
  return (
    <div className="w-full max-w-md mt-4 bg-gray-50 border border-gray-200 rounded-lg shadow-sm p-4">
      <div className="flex justify-between items-center mb-2">
        ...
        <button onClick={handleCopy} className="ml-2 bg-blue-600 text-white text-xs px-3 py-1 rounded hover:bg-blue-700 transition-colors">Copy Script</button>
      </div>
      <pre className="font-mono text-base text-gray-800 whitespace-pre-wrap leading-relaxed break-words">{script}</pre>
    </div>
  );
}
    ...
    <div className="flex flex-col h-full flex-grow min-h-[500px] bg-white shadow-md hover:shadow-lg transition-shadow rounded-xl p-6 w-full max-w-md mx-auto relative justify-between">
      ...
      <div className="flex flex-col flex-grow justify-between h-full">
        <div>
          ...
          {legislator.district && (<div className="text-gray-700">District: {legislator.district}</div>)}
        ...
        <div className="mt-4">
          <ScriptBox title={title === "Representative" ? "Rep." : "Senator"} lastName={lastName} zip={zip} role={title === "Representative" ? "Rep." : "Senator"} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-screen-xl mx-auto flex flex-col items-center gap-8">
        ...
        {error && (
          <div className="bg-red-100 text-red-700 rounded-lg p-4 text-center w-full max-w-md">Couldn't find district info for this ZIP.</div>
        )}
        {!loading && !error && data && (
          <>
